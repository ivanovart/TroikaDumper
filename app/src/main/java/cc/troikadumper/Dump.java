...
public class Dump {
    ...
    public static Dump fromFile(File file) throws IOException {
        ...
        byte[] uid = HexUtils.INSTANCE.fromString(scanner.nextLine());

        byte[][] data = new byte[BLOCK_COUNT][BLOCK_SIZE];
        for (int i = 0; i < BLOCK_COUNT; i++) {
            data[i] = HexUtils.INSTANCE.fromString(scanner.nextLine());
        }
    ...
    protected void parse() {
        ...
        if (lastUsageDay > 0) {
            ...
            c.set(1992, 1, 1, lastUsageHour, lastUsageMinute);
    ...
    public void write(Tag tag) throws IOException {
        ...
        if (!Arrays.equals(tag.getId(), this.getUid())) {
            throw new IOException("Card UID mismatch: \n"
                    + HexUtils.INSTANCE.toString(tag.getId()) + " (card) != "
                    + HexUtils.INSTANCE.toString(getUid()) + " (dump)");
        }
    ...
    public String getUidAsString() {
        return HexUtils.INSTANCE.toString(getUid());
    }
    ...
    public String[] getDataAsStrings() {
        ...
        for (int i = 0; i < data.length; i++) {
            blocks[i] = HexUtils.INSTANCE.toString(data[i]);
        }
        return blocks;
    }
